...
int main() {
  ...
  while (getline(in_map_, line)) {
	...
	// Define tthe reference velocity to ccontrol your velocity.
  ...
  h.onMessage([&ref_vel,&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		// Define the lane in which we are riding.
		int lane = 1;
		...
		for(int i=0;i<sensor_fusion.size();i++)
		{
			...
			} else if (lane != 2 && d > (4*(lane+1)) && d < (4*(lane+2))) 
			{
				...
				if ((check_car_s > (car_s - 40)) && (check_car_s < (car_s + 40))) 
				{
		...
		// Shifting map coordinate to car coordinate
